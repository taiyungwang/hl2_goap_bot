...
Navigator::Navigator(Blackboard& blackboard, float targetRadius) :
...
bool Navigator::navigate() {
	...
	return  (!moveCtx->hasGoal() && reachedGoal())
			|| moveCtx->isStuck() || (!moveCtx->hasGoal() && !moveCtx->reachedGoal()); // stuck
}

bool Navigator::buildPath(const Vector& targetLoc) {
	finalGoal = UTIL_FindGround(targetLoc);
	moveCtx->stop();
	const Player* self = moveCtx->getBlackboard().getSelf();
	CNavArea* area = TheNavMesh->GetNavArea(finalGoal);
	if (area == nullptr) {
...
bool Navigator::reachedGoal() {
	return path.Count() == 0
			&& (moveCtx->getBlackboard().getSelf()->getCurrentPosition()
...
void Navigator::getNextArea() {
	auto& blackboard = moveCtx->getBlackboard();
	auto* self = blackboard.getSelf();
	CNavArea* startArea = getCurrentArea(self->getCurrentPosition());
